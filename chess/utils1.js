...
function move(game, packet, move, piece) {
	if (move[0] === packet.newX && move[1] == packet.newY) {
...
function updateAllowedMoves(game, color) {
	for (let p = 0; p < game.pieces.length; p++) {
		...
		for (let m = 0; m < moves.length; m++) {
			...
			if (move(gameCopy, packet, moves[m], gameCopy.pieces[p])) {
...
module.exports = {isCheckmateOrDraw, checkCollision, isCheck, updateAllowedMoves};
